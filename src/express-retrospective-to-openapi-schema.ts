    ...
    paths: Object.fromEntries(
      expressRetroSpectiveResult
        ...
        .map(({ path, handlers, method, params }) => {
          const validationMiddleware = handlers.find(isValidationMiddleware) as ReturnType<typeof validate>;
          const operationObject: OpenAPIV3.OperationObject = {
            ...
            parameters: [
              ...
              ...(validationMiddleware.zodSchemas.query
                ? Object.entries<ZodString>(validationMiddleware.zodSchemas.query.shape).map(
                    ([paramName, paramZodShape]) => ({
                      name: paramName,
                      in: 'query',
            ...
            ...(validationMiddleware.zodSchemas.body
              ? {
                  requestBody: {
                    ...
                    content: {
                      'application/json': {
                        schema: zodToJsonSchema(validationMiddleware.zodSchemas.body, {
                          target: 'openApi3',
                        }),
                      },
                    },
              ...
              ...(validationMiddleware.zodSchemas.response
                ? {
                    '200': {
                      ...
                      content: {
                        'application/json': {
                          schema: zodToJsonSchema(validationMiddleware.zodSchemas.response, {
                            target: 'openApi3',
                          }),
                        },
                      },
                ...
                : {}),
              ...Object.fromEntries(
                Object.entries(validationMiddleware.zodSchemas.statusCodes).map(([statusCode, zodObject]) => [
                  statusCode,
                  {
                    description: '',
                    content: {
