...
type InferShapeOrZodTypeIfNotUndefined<T> = T extends ZodRawShapeOrObjOrArray ? InferZodRawShapeOrZodType<T> : unknown;
...
function isZodObject(zodRawShapeOrZodType: ZodRawShapeOrObjOrArray): zodRawShapeOrZodType is ZodObject<any> {
...
export const validate: ValidationMiddleware = (options) => {
  ...
  const zodQuerySchema: ZodObject<any> | undefined =
    optionsWithDefaults.query && optionsWithDefaults.queryThrowsError ? z.object(optionsWithDefaults.query) : undefined;
  ...
  const handler: RequestHandler<any, any, any, any> & ValidationMiddlewareProperties = (req, res, next) => {
